...
import a2019.main.DataSetInfo;
import a2019.main.FS;
import a2019.main.SettingForGA;
import jfml.FuzzyInferenceSystem;
import jfml.JFML;
import jfml.term.FuzzyTermType;

public class CalcConclusion2 {
	public static void main(String[] args) {
//		String inputFML = args[0];
//		int calcGeneration = Integer.parseInt(args[1]);
//		int outputInterval = Integer.parseInt(args[2]);

		String inputFML = "ex_seed2019_Gene9_evaSize500_popRB10_popKB10_ruleNum2000";
		...
		newdir.mkdirs();
		newdir = new File(folderName + "/reasoning");
		newdir.mkdirs();
		...
		calcMSE(fsPop, folderName + "/reasoning/gene0_", setting, tra, tst, allMSE);
		...
		for(int gene_i = 0; gene_i < calcGeneration; gene_i++) {
			...
			outputXML(fileName, fsPop);
			calcMSE(fsPop, folderName + "/reasoning/gene" + String.valueOf((gene_i+1) * outputInterval) + "_", setting, tra, tst, allMSE);
	...
	public static void calcMSE(FS fsPop, String fileName, SettingForGA setting, DataSetInfo tra, DataSetInfo tst, ArrayList<float[]> allMSE) {

		fsPop.makeFS(setting);
		float[] yTra = fsPop.reasoning(setting, tra);
		float[] yTst = fsPop.reasoning(setting, tst);
		outputReasoning(fileName, yTra, yTst);

		float traMSE = a2019.main.FmlGaManager.calcMSE(yTra, tra);
		float tstMSE = a2019.main.FmlGaManager.calcMSE(yTst, tst);
		float[] newMSE = {traMSE, tstMSE};
		allMSE.add(newMSE);
	}

	public static void outputReasoning(String _fileName, float[] yTra, float[] yTst) {
		int traDataSize = yTra.length;
		int tstDataSize = yTst.length;

		try {
			String fileName = _fileName + "tra.csv";
			FileWriter fw = new FileWriter(fileName, true);
			PrintWriter pw = new PrintWriter( new BufferedWriter(fw) );
			pw.println("data,tra");
			for(int data_i = 0; data_i < traDataSize; data_i++) {
				pw.println(data_i + "," + yTra[data_i]);
			}
			pw.close();

			fileName = _fileName + "tst.csv";
			fw = new FileWriter(fileName, true);
			pw = new PrintWriter( new BufferedWriter(fw) );
			pw.println("data, tst");
			for(int data_i = 0; data_i < tstDataSize; data_i++) {
				pw.println(data_i + "," + yTst[data_i]);
			}
	...
	public static void outputMSE(String fileName, int outputInterval, ArrayList<float[]> allMSE) {
		...
		try {
			...
			for(int gene_i = 0; gene_i < generation; gene_i++) {
				pw.print(String.valueOf(gene_i * outputInterval) + ",");
