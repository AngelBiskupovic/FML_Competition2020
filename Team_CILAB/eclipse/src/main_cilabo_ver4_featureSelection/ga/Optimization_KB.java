package main_cilabo_ver4_featureSelection.ga;
...
import main.Item;
import main.ItemSet;
import main_cilabo_ver4_featureSelection.CILAB_Main;
import main_cilabo_ver4_featureSelection.Main;
import main_cilabo_ver4_featureSelection.TSK;
...
//		outputXML(dirGeneration, population);
		...
		// ************************************************************
		/* 3. Optimization Loop */
		for(int generation = 1; generation <= Main.Generation_KB; generation++) {
			System.out.println();
			System.out.println("************************");
			...
			for(int pop = 0; pop < Main.N_KB; pop++) {
				System.out.print(pop+1);
			...
			if(generation == Main.Generation_KB) {
				// Population
//				outputXML(dirGeneration, population);
			}
			else {
...
//				outputXML(dirGeneration, population.get(0));
	...
	public static void initialize(ArrayList<Individual_KB> population, float[][][] KBbest) {
		...
		for(int pop = 1; pop < Main.N_KB; pop++) {
	...
	public static float[][][] perturbing(float[][][] original) {
		...
		else if(Main.termType == FuzzyTermType.TYPE_triangularShape) {
			newParams = perturbingForTriangularShape(original);
		}
	...
	//TODO
	public static float[][][] perturbingForTriangularShape(float[][][] original){
		// ************************************************************
		float[][][] newParams = new float[Main.dimension][Main.H][Main.paramLength];
		for(int n = 0; n < Main.dimension; n++) {
			// ************************************************************
			// Collect center vertex
			float[] vertices = new float[Main.H];
			for(int h = 0; h < Main.H; h++) {
				vertices[h] = original[n][h][1];	// Param2
			}
			...
			float denominator = 50;	// The range of [0, 0.02]
			float delta = rndDelta(denominator);
			float newValue = vertices[0] + delta;
			if(newValue < 0) points.add(0f);
			else if(newValue > vertices[1]) points.add(vertices[0]);
			else points.add(newValue);

			for(int h = 1; h < (Main.H - 1); h++) {
				delta = rndDelta(denominator);
				newValue = vertices[h] + delta;
				if(newValue < vertices[h-1]) points.add(vertices[h]);
				else if(newValue > vertices[h+1]) points.add(vertices[h]);
				else points.add(newValue);
			}

			delta = rndDelta(denominator);
			newValue = vertices[Main.H-1] + delta;
			if(newValue < vertices[Main.H-1-1]) points.add(vertices[Main.H-1]);
			else if(newValue > 1) points.add(1f);
			else points.add(newValue);

			points.add(1f);

			// ************************************************************
			// Transform newVertices into newParams
			for(int h = 0; h < Main.H; h++) {
				for(int i = 0; i < Main.paramLength; i++) {
					newParams[n][h][i] = points.get(h + i);
				}
			}
		}
	...
	public static float[][][] perturbingForGaussianShape(float[][][] original) {
		...
		for(int n = 0; n < Main.dimension; n++) {
			// ************************************************************
			// Mean Perturbation
			float delta = rndDelta(50);		// The range of [0, 0.02]
			...
			for(int h = 1; h < (Main.H - 1); h++) {
				delta = rndDelta(50);		// The range of [0, 0.02]
	...
	public static float rndDelta(float denominator) {
		int direction = 1;
		if(CILAB_Main.rnd.nextBoolean()) {
			direction = -1;
		}
		...
		if(CILAB_Main.rnd.nextFloatIE() < Pp) {
			// Do Perturbation
			delta = CILAB_Main.rnd.nextFloatII() / denominator;
		}
