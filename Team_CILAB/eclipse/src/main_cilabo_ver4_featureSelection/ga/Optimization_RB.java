...
//		outputXML(dirGeneration, population);
		...
		// ************************************************************
		/* 3. Optimization Loop */
		for(int generation = 1; generation <= Main.Generation_RB; generation++) {
			System.out.println();
			System.out.println("************************");
			...
			for(int pop = 0; pop < Main.N_RB; pop++) {
				System.out.print(pop+1);
			...
			if(generation == Main.Generation_RB) {
				// Population
//				outputXML(dirGeneration, population);
			}
			else {
...
//				outputXML(dirGeneration, population.get(0));
	...
	public static void initialize(ArrayList<Individual_RB> population, int[][] RBbest, ItemSet Dtra, KnowledgeBaseType kb) {
		...
		for(int pop = POP_START; pop < Main.N_RB; pop++) {
