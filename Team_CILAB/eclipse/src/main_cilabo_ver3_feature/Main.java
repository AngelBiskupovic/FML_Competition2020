...
//	public static String[] termName = {"Small", "Medium", "Large"};
	...
	public static void main(String[] args) {
		...
		for(int loop = 885; loop < Math.pow(2, nameCandidate.length); loop++) {
			/* Make Flag for Used Feature */
			String binary = Integer.toBinaryString(loop);
			while(binary.length() != nameCandidate.length) {
				binary = "0" + binary;
			}

			/* Make Result Directry */
			dir = dirName + sep + binary;
			newdir = new File(dir);
			newdir.mkdirs();

			/* Make Array for Used Feature */
			ArrayList<String> usedFeatures = new ArrayList<>();
			for(int i = 0; i < nameCandidate.length; i++) {
				if(binary.charAt(i) == '1') {
					usedFeatures.add(nameCandidate[i]);
				}
			}
			name = usedFeatures.toArray(new String[0]);
			dimension = name.length;

			/* Optimizing Fuzzy Inference System based on Genetic Algorithm */
			FuzzyInferenceSystem fs = CILAB_Main.generalFramework(train);

			/* Inference Dataset */
			float traMSE = inference(fs, train, dir + sep + outputName + "_train.csv");
			float tstMSE = inference(fs, test, dir + sep + outputName + "_test.csv");

			/* 7. Output XML file */
			String xmlFile = dir + sep + outputName + "_" + binary + ".xml";
